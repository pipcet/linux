...
dma_addr_t apple_dcp_fb_alloc_buffer(struct apple_dcp_fb *apple,
...
EXPORT_SYMBOL(apple_dcp_fb_alloc_buffer);

static int apple_dcp_fb_probe(struct platform_device *pdev)
{
	struct apple_dcp_fb *apple;
	int ret;
...
static int apple_dcp_fb_remove(struct platform_device *pdev)
{
	return -EBUSY;
}

static const struct of_device_id of_match[] = {
	{ .compatible = "apple,dcp-framebuffer" },
	{}
};
MODULE_DEVICE_TABLE(of, of_match);

static struct platform_driver apple_platform_driver = {
	.probe	= apple_dcp_fb_probe,
	.remove	= apple_dcp_fb_remove,
	.driver	= {
		.name = "apple-dcp-fb",

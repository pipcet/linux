			 ...
			 const struct blk_mq_queue_data *bd)
{
	...
	apple_nvme_submit_cmd(nvmeq, cmnd, bd->last);
	return BLK_STS_OK;
out_unmap_data:
...
static void apple_nvme_submit_async_event(struct nvme_ctrl *ctrl)
{
	...
	apple_nvme_submit_cmd(dev->adminq, &c, true);
}
 ...
 * apple_nvme_poll(). This is the last chance for the driver to see a natural
 * completion before nvme_cancel_request() terminates all incomplete requests.
 */
static void apple_nvme_reap_pending_cqes(struct apple_nvme_dev *dev)
{
	spin_lock(&dev->ioq->cq_poll_lock);
	nvme_process_cq(dev->ioq);
	spin_unlock(&dev->ioq->cq_poll_lock);
}
...
static void apple_nvme_dev_disable(struct apple_nvme_dev *dev, bool shutdown)
{
	...
	apple_nvme_reap_pending_cqes(dev);
...
static int apple_nvme_probe(struct platform_device *pdev)
{
	...
	result = nvme_init_ctrl(&dev->ctrl, &pdev->dev, &nvme_ctrl_ops, 0);
	if (result)
		goto out_unmap;

	dev->cl.dev = &pdev->dev;
	result = PTR_ERR_OR_ZERO(mbox_request_channel(&dev->cl, 0));
	if (result)
		goto out_unmap;

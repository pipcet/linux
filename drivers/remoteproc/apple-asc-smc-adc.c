...
{
	static u8 invalid_reading[] = { 0x58, 0xf7, 0xc6, 0x10, 0, 0, 0, 0 };
	struct adc *adc = ptr;
	switch (adc->state) {
	case ADC_READY:
		BUG_ON(!adc->prop);
...
static int adc_read(struct kvbox *kvbox, struct kvbox_prop *prop)
{
	...
	adc->prop = prop;
	char *smc_key = kasprintf(GFP_KERNEL, "aDCR-%08x", key);
	printk("using smc_key %s\n", smc_key);
	u64 data;
	struct kvbox_prop smc_prop = {
		.key = smc_key,
		.key_len = strlen(smc_key),
		.data = prop->data,
		.data_len = prop->data_len,
	};
	memcpy(&adc->smc_prop, &smc_prop, sizeof(smc_prop));
	kvbox_read(adc->smc, &adc->smc_prop, adc_callback, adc);
